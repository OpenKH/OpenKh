...
The list below has been generated by looking at [00place.bin and place.bin](file/type/place).

Where nothing is specified, means that one of the two files does not contain any information of that map at all.

Note that `00place` and `place` might use slightly different [message ID](file/type/msg) to describe their maps in-game.
