...
namespace OpenKh.Tests.Engine
{
    ...
    {
        [Fact]
        public void ShouldGetImmutableMesh()
        {
            var alaxi = new int[]
            {
            ...
            var immutableMesh = VU1Simulation.Run(data, 0, 123, alaxi);

            Assert.NotNull(immutableMesh);
            Assert.Equal(123, immutableMesh.textureIndex);
            Assert.Equal(0x4c, immutableMesh.vertexAssignmentsList.Length);
            Assert.Equal(0x4f, immutableMesh.indexAssignmentList.Length);

            AssertEqual(immutableMesh.vertexAssignmentsList[0][0],
                0, 0x35, 11.55658f, -10.695402f, -1.8525791f, 0);
            AssertEqual(immutableMesh.vertexAssignmentsList[1][0],
                0, 0x35, 12.341873f, -6.8589644f, -6.79047f, 0);
            AssertEqual(immutableMesh.vertexAssignmentsList[75][0],
                0, 0x16, -0.29302156f, -0.004016876f, 13.689127f, 0);

            AssertEqual(immutableMesh.indexAssignmentList[0],
                0x10, 0x27, 0f, 0.26171875f);
            AssertEqual(immutableMesh.indexAssignmentList[1],
                0x10, 0x28, 0.02734375f, 0.24609375f);
            AssertEqual(immutableMesh.indexAssignmentList[78],
                0x30, 0x36, 0.5234375f, 0.49609375f);
        }
        ...
        [Fact]
        public void ShouldGetImmutableMeshHighDef()
        {
            var alaxi = new int[]
            {
            ...
            Assert.NotNull(immutableMesh);
            Assert.Equal(123, immutableMesh.textureIndex);
            Assert.Equal(0x4a, immutableMesh.vertexAssignmentsList.Length);
            Assert.Equal(0x4f, immutableMesh.indexAssignmentList.Length);

            AssertEqual(immutableMesh.vertexAssignmentsList[0][0],
                1f, 0x89, 6.9705353f, 0.5387306f, -4.146785f, 1f);
            AssertEqual(immutableMesh.vertexAssignmentsList[1][0],
                1f, 0x89, 6.1054993f, 1.1867695f, -3.7489133f, 1f);
            AssertEqual(immutableMesh.vertexAssignmentsList[73][0],
                0.5f, 0x8c, 9.300911f, -0.66315675f, -5.8931007f, 0.5f);
            AssertEqual(immutableMesh.vertexAssignmentsList[73][1],
                0.5f, 0xd3, 3.4593887f, -1.39147f, -1.8666306f, 0.5f);
        }
        ...
        private static void AssertEqual(VertexAssignment v,
            float expectedWeight,
            int expectedMatrixIndex,
            float expectedX,
            float expectedY,
            float expectedZ,
            float expectedW)
        {
            Assert.Equal(expectedWeight, v.weight);
            Assert.Equal(expectedMatrixIndex, v.matrixIndex);
            Assert.Equal(expectedX, v.rawPos.X);
            Assert.Equal(expectedY, v.rawPos.Y);
            Assert.Equal(expectedZ, v.rawPos.Z);
            Assert.Equal(expectedW, v.rawPos.W);
        }

        private static void AssertEqual(IndexAssignment v,
            int expectedVertexFlag,
            int expectedIndexToVertexAssignment,
            float expectedU,
            float expectedV)
        {
            Assert.Equal(expectedVertexFlag, v.vertexFlag);
            Assert.Equal(expectedIndexToVertexAssignment, v.indexToVertexAssignment);
            Assert.Equal(expectedU, v.uv.X);
            Assert.Equal(expectedV, v.uv.Y);
        }
